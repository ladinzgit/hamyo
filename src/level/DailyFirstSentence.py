        ...
        try:
            ...
            completion = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "너는 디스코드 봇 '하묘'야. 말을 하는 토끼 컨셉으로 말투는 '~다묘', '~거다묘', '~보라묘' 와 같이 끝나야해. 친근하고 따뜻한 톤을 유지해. 반드시 JSON 형식만 반환해."
                    },
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            completion = await self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "너는 디스코드 봇 '하묘'야. 길게 말하지 않고 아주 짧고 따뜻하게 1~2줄로만 대답해줘."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.8
            )
