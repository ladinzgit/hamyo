...
class FortuneCommand(commands.Cog):
    ...
    async def tell_fortune(self, ctx):
        ...
        if not birthday:
            await ctx.reply("생일 정보가 없다묘! <#1396829221741002796>에서 생일을 등록해달라묘.")
            return
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "- 그 문장의 양식은 **행운의 상징: (행운의 상징)** 으로 시작해서 알려줘. 이때는 문장이 아닌 단어를 사용해. 이 행운의 상징에는 '묘'를 붙이지 않아도 돼. 행운의 상징이 여러 개일시, comma하고 띄어쓰기(, )로 구분해줘.\n"
                            "- 이때는 볼드 처리를 해줘."
                        ),
                    },
